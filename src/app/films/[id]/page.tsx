    ...
    try {
      ...
      if (storedEmail) {
        const likedFilmsResponse = await fetch(
          `http://localhost:3010/user/videoclub/likes/${storedEmail}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
        ...
        const likes = await likedFilmsResponse.json();
        console.log(likes);
        likes.forEach((item: { id: string; email: string }) => {
  ...
  const handleLike = async () => {
    try {
      const url = liked
        ...
        : `http://localhost:3010/films/like/${slug}`;
